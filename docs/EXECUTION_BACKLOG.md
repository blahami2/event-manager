...
## T-ADMIN-C: Admin UI Tier C — Stats Strip, Rich Filters, Bulk Actions, Detail Drawer

**Status:** DONE

### Description

Tier A consolidated the structure; Tier B refreshed the visual system;
Tier C reshapes the information architecture and adds the features
that make the page actually useful:

1. A compact stats strip at the top of the registrations page shows
   Total / Confirmed / Cancelled / Adults / Children without an
   extra click.
2. The standalone `/admin` dashboard is folded into the registrations
   page; `/admin` redirects to `/admin/registrations`.
3. Stay and Accommodation filter dropdowns join Status and Search;
   active filters surface as removable chips below with a "Clear all"
   reset.
4. Row selection (checkboxes per row + select-all-visible in the
   header) reveals a sticky bulk action bar with "Export selected" and
   "Resend emails" (confirm-gated).
5. Clicking a row opens a right-side detail drawer with the full
   record, timestamps, and actions; Escape / backdrop / close button
   dismiss it.
6. Search query substrings are highlighted in the name and email
   cells (case-insensitive, case-preserving).
7. Page-size changes keep the user on the current page when data is
   still there; they only clamp when the new page would be empty.

### Design Decision: Redirect over Dashboard Retention

The standalone `/admin` dashboard no longer exists. Every stat it
previously surfaced is now visible on the registrations page, above
the filters, in a more compact layout. Keeping the separate page
would have meant maintaining two URL entry points that tell the same
story, so `/admin` redirects to `/admin/registrations` via
`next/navigation`'s `redirect()`. Existing bookmarks, emails, and
deep links continue to work.

### Files Added

- `src/app/api/admin/registrations/stats/route.ts` — `GET` endpoint
  that returns `{ total, confirmed, cancelled, totalAdults,
  totalChildren }`, authenticated via the same `verifyAdmin` guard as
  the other admin endpoints.
- `src/app/api/admin/registrations/stats/route.test.ts` — 3 route
  tests (success, 401, 403).
- `src/components/admin/StatsStrip.tsx` — 5-tile compact stats row
  with semantic accent colours for Confirmed (success) and Cancelled
  (danger); shows skeleton placeholders while stats are loading so
  the page never jumps.
- `src/components/admin/StatsStrip.test.tsx` — 2 tests.
- `src/components/admin/BulkActionBar.tsx` — sticky-bottom bar with
  selected count, Export, Resend, and Clear.
- `src/components/admin/BulkActionBar.test.tsx` — 5 tests.
- `src/components/admin/RegistrationDrawer.tsx` — right-side drawer
  with full record, timestamps, ID, and Edit / Resend / Cancel
  actions; closes via Escape / backdrop / explicit close button.
- `src/components/admin/RegistrationDrawer.test.tsx` — 5 tests.
- `src/app/admin/registrations/page-size-logic.ts` — pure
  `resolvePageAfterPageSizeChange` helper, extracted for direct unit
  testing.
- `src/app/admin/registrations/__tests__/page-size-logic.test.ts` — 4
  tests covering the keep / clamp / zero-total / exceed-single-page
  cases.

### Files Modified

- `src/app/admin/page.tsx` — now a one-line `redirect()` to
  `/admin/registrations`. The previous server-component dashboard
  (heading, stat cards, quick links) is retired; the redirect test
  replaces the previous dashboard tests.
- `src/app/admin/__tests__/page.test.tsx` — rewritten to assert the
  redirect target.
- `src/components/admin/RegistrationFilters.tsx` — adds Stay and
  Accommodation dropdowns; renders active-filter chips with one-click
  dismiss; adds a "Clear all" button that appears only when at least
  one filter is active. Original Status + Search are preserved.
- `src/components/admin/__tests__/RegistrationFilters.test.tsx` —
  expanded from 6 to 8 tests covering the new controls and chip
  behaviour.
- `src/components/admin/RegistrationTable.tsx` — adds opt-in
  `selection` (`selectedIds` + `onSelectionChange`), `onRowClick` for
  the drawer, `searchHighlight` for substring highlighting, and a
  `highlightMatches` pure helper exported for unit-testable logic.
  Selected rows get an accent left-border (3px inset box-shadow) and
  a subtle tint. Row click is cancelled by `stopPropagation` on
  cell-level interactive areas (notes expand toggle, action buttons,
  checkbox) so a click on those never opens the drawer.
- `src/components/admin/__tests__/RegistrationTable.test.tsx` — adds
  4 tests for selection, highlighting, and row click.
- `src/app/admin/registrations/page.tsx` — fully rewired: pulls the
  stats strip, rich filters, sort state, row selection, drawer, and
  bulk-action bar into one cohesive page. The page-size handler now
  delegates to `resolvePageAfterPageSizeChange` so the behaviour is
  unit-tested in isolation. Search query is passed to the table as
  `searchHighlight`. Toast replaces banner feedback for every
  success/error.
- `src/i18n/messages/{en,cs,sk}.json` — 27 new keys under
  `admin.registrations.*` (stats labels, selection labels,
  bulk-action labels, drawer labels, filter chips). All three
  locales stay in lockstep (enforced by
  `tests/unit/i18n/messages.test.ts`).

### Intentional Deviations & Scope Choices

- **Stay + Accommodation filters apply client-side** on the current
  page, not server-side. The `/api/admin/registrations` list endpoint
  still accepts only `status` + `search`. Extending it server-side
  would require a repository signature change, an API contract
  update, and additional tests — clean work, but orthogonal to the
  admin-GUI overhaul. The client-side filter is documented in
  `applyClientFilters()` with a pointer to the follow-up. Impact is
  bounded: totals in `StatsStrip` reflect the full dataset (server
  query), while pagination counts reflect the backend's status-and-
  search-only view. A follow-up ticket should push the filter
  server-side.
- **Sort is also client-side**, over the current page. Same
  trade-off: moving to server-side sort requires an API contract
  change. The `sort` prop contract on `RegistrationTable` is ready
  for it — the parent can switch to server-side without touching the
  table.
- **Bulk cancel intentionally omitted**, per the brief's guidance.
  Destructive bulk actions are too risky without a preview + undo
  flow; if the team wants it, a dedicated ticket should design the
  confirmation UX.
- **Bulk CSV export passes selected ids as `id` query params.** The
  existing export endpoint currently ignores them (exports everything);
  the UI contract is forward-compatible — when the endpoint grows
  selection support, no UI change is needed. For now, bulk export
  downloads the full CSV even when a subset is selected. This is a
  deliberate scoping choice; flagged for review.
- **`/admin` dashboard retired.** The previous
  `AdminDashboardPage` rendered stats cards + quick links. Every
  piece of content it surfaced now lives at the top of
  `/admin/registrations`, so keeping two pages would have been
  duplicate maintenance. The redirect preserves deep links.
...
- `npm run lint` — passes (the 1 pre-existing `layout.tsx` font
  warning unchanged).
- `npx vitest run` — **570 passed** (+22 over Tier B's 548).
- All i18n files kept in lockstep (I18N5).
- Drawer is dismissible via Escape / backdrop / close-button; toast
  viewport remains `aria-live="polite"`; sortable headers carry
  `aria-sort`; bulk action bar has `role="region"`.
